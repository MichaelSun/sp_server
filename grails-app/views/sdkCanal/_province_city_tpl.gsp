...
<g:each in="${pList}" var="p">
	<div
		class="fieldcontain ${hasErrors(bean: sdkCanalInstance, field: 'area', 'error')} ">
		...
		<g:checkBox name="province" value="${p.key}" id="${p.key }_id"
			checked="${sdkCanalInstance?.area?.containsKey(p.key)==true}"
			onclick="show('${p.key }')" />
	...
	<div id="${p.key}"
		style="${sdkCanalInstance?.area?.containsKey(p.key)?"display: block;":"display:none;"}"
		class="fieldcontain ${hasErrors(bean: sdkCanalInstance, field: 'area', 'error')}"
		align="right">
		...
		<g:each in="${p.value}" var="c">
			...
			<g:checkBox name="area.${p.key }" value="${c}"
				checked="${sdkCanalInstance?.area?.get(p.key)?.contains(c)==true}" />
		</g:each>
	</div>
