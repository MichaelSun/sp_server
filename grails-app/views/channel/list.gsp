		...
		<div id="list-channel" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					...
					<g:sortableColumn property="mainCode" title="${message(code: 'channel.mainCode.label', default: 'Main Code')}" />
					
						<%--<g:sortableColumn property="subCode" title="${message(code: 'channel.subCode.label', default: 'Sub Code')}" />--%>
						<g:sortableColumn property="rate" title="${message(code: 'channel.rate.label', default: 'Rate')}" />
				...
				<g:each in="${channelInstanceList}" status="i" var="channelInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${channelInstance.id}">${fieldValue(bean: channelInstance, field: "name")}</g:link></td>
												<td>${fieldValue(bean: channelInstance, field: "mainCode")}</td>
						
												<%--<td>${fieldValue(bean: channelInstance, field: "subCode")}</td>
						
						--%><td>${fieldValue(bean: channelInstance, field: "rate")}</td>
						<td>${fieldValue(bean: channelInstance, field: "dateCreated")}</td>
