...
<g:each in="${pList}" var="p">
	<div
		class="fieldcontain ${hasErrors(bean: smsCanalInstance, field: 'area', 'error')} ">
		...
		<g:checkBox name="province" value="${p.key}" id="${p.key }_id"
			checked="${smsCanalInstance?.area?.containsKey(p.key)==true}"
			onclick="show('${p.key }')" />
	...
	<div id="${p.key}"
		style="${smsCanalInstance?.area?.containsKey(p.key)?"display: block;":"display:none;"}"
		class="fieldcontain ${hasErrors(bean: smsCanalInstance, field: 'area', 'error')}"
		align="right">
		...
		<g:each in="${p.value}" var="c">
			...
			<g:checkBox name="area.${p.key }" value="${c}"
				checked="${smsCanalInstance?.area?.get(p.key)?.contains(c)==true}" />
		</g:each>
	</div>
